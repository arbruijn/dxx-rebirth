...
long SeekHead::Parse() {
  ...
  ptrdiff_t count_ = (pEntry - m_entries);
  assert(count_ >= 0);
  assert(static_cast<long long>(count_) <= entry_count);

  m_entry_count = static_cast<int>(count_);

  count_ = (pVoidElement - m_void_elements);
  assert(count_ >= 0);
...
long Track::Seek(long long time_ns, const BlockEntry*& pResult) const {
  ...
  while (lo < hi) {
    ...
    assert(pCluster);
    assert(pCluster->GetIndex() >= 0);
    assert(pCluster->GetIndex() == (mid - m_pSegment->m_clusters));
...
long Block::Parse(const Cluster* pCluster) {
  ...
  const int lacing = (m_flags & 0x06) >> 1;
...
bool Block::IsInvisible() const { return ((m_flags & 0x08) != 0); }

Block::Lacing Block::GetLacing() const {
  const int value = (m_flags & 0x06) >> 1;
