...
namespace uvpx
{
    ...
    Player::LoadResult VideoPlayer::load(const char *fileName, int audioTrack, bool preloadFile)
    {
        ...
        return load(SDL_RWFromFile(videoFullPath.c_str(), "rb"), audioTrack, preloadFile);
	}
    ...
    */
    Player::LoadResult VideoPlayer::load(SDL_RWops *file, int audioTrack, bool preloadFile)
    {
        reset();

        // open file
        m_reader = new FileReader();
        if (m_reader->open(file, preloadFile))
        {
            debugLog("Failed to open video file");
        ...
        while (track_num != num_tracks)
        {
            ...
            if (trackType == mkvparser::Track::kAudio)
            {
                ...
                if (audioTrack == currentAudioTrack - 1)
                {
                    ...
                    size_t size;
                    unsigned char * data = const_cast<unsigned char *>(pAudioTrack->GetCodecPrivate(size));
